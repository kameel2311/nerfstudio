...
from nerfstudio.cameras.custom_cameras import CLCameras
...
@dataclass
class NerfstudioCLDataParserConfig(DataParserConfig):
    ...
    path_to_transform: Path = Path()
    """Path to the transform file. This Transform should contain the matrix and scale from the Mapping Session."""
...
class Nerfstudio(DataParser):
    ...
    config: NerfstudioCLDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        # Load the precomputed transform from the mapping session
        if self.config.path_to_transform.suffix == ".json":
            CONSOLE.log(f"[bold yellow]Loading transform from {self.config.path_to_transform}")
            mapping_transform = load_from_json(self.config.path_to_transform)
        else:
            CONSOLE.log(f"[bold yellow]Loading transform from {self.config.path_to_transform / 'dataparser_transforms.json'}")
            mapping_transform = load_from_json(self.config.path_to_transform / "dataparser_transforms.json")
        ...
        transform_matrix = torch.tensor(mapping_transform["transform"], dtype=torch.float32)
        transform_matrix = torch.cat([transform_matrix, torch.tensor([[0, 0, 0, 1]], dtype=transform_matrix.dtype)], 0)
        poses = transform_matrix @ poses 

        # # Adding Override Logic for transformation and Scale
        # if "override_transform" in meta:
        #     override_transform = torch.tensor(meta["override_transform"], dtype=poses.dtype)
        #     override_transform = torch.cat(
        #         [override_transform, torch.tensor([[0, 0, 0, 1]], dtype=override_transform.dtype)], 0
        #     )
        #     poses = override_transform @ poses 
        #     CONSOLE.log(f"[yellow] Dataset is overriding transform matrix to {override_transform}")
        #     transform_matrix = override_transform

        # Scale poses
        # scale_factor = 1.0
        # if "override_scale" in meta:
        #     override_scale = float(meta["override_scale"])
        #     scale_factor = override_scale
        #     CONSOLE.log(f"[yellow] Dataset is overriding scale factor to {scale_factor}")

        scale_factor = 1.0
        scale_factor *= float(mapping_transform["scale"])
        ...
        if applied_transform is not None:
            ...
            dataparser_transform_matrix = dataparser_transform_matrix[:-1,:] # Removing last row of the matrix
